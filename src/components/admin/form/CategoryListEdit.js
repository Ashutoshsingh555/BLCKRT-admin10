...
export default class CategoryListEdit extends React.Component {
   constructor() {
         super();
         this.state = {
            data : [],
    ...
    getcate = () => {
      this.setState({categoryId:this.props.data,subcategoryId:this.props.sdata})
                ...
                {item.SubCategories.map((subval,idx) => 
                        <div className="size-box-brand mx-4">
                          <input className="form-check-input bg-outlin-danger px-4 mt-1 my-2" 
                                ...
                                value={subval.id}
                                id={item.id}
                                checked={ this.state.subcategoryId === subval.id &&  this.state.categoryId === item.id}
